        ...
        for (int i=1;i<=1;i++) {
            for (int j=1;j<=5;j++) {
                if(lesson.getWhat_day()==i && lesson.getWhich_segment()==j) {
                    LinearLayout.LayoutParams one_cell = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
                    ...
                    mcellViews.get(0).addView(textView);
                    lesson_count++;
